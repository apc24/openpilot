    ...
    try:
      ...
      try:
        ...
        if e2e_alive and e2e_valid:
          ...
          if CC.latActive and hasattr(e2e_data, 'steeringAngleDeg'):
            actuators.steeringAngleDeg = float(e2e_data.steeringAngleDeg)
            actuators.steer = float(0.0)
          return True
  ...
  def state_control(self, CS):
    ...
    if CC.longActive or CC.latActive:
      if e2e_applied:
        ...
        cloudlog.info("DUAL INFERENCE - E2E Active: ")
        cloudlog.info(f"std_accel={std_accel_str}, e2e_accel={actuators.accel:.4f}")
        cloudlog.info(f"std_steer={std_steer_str}, e2e_steer={actuators.steer:.4f}")
        cloudlog.info(f"std_angle={std_angle_str}, e2e_angle={actuators.steeringAngleDeg:.2f}")
      else:
        # 標準制御時のログ出力
        cloudlog.debug(f"STANDARD CONTROL: longActive={CC.longActive}, latActive={CC.latActive}")
        cloudlog.debug(f"accel={actuators.accel:.4f}, steer={actuators.steer:.4f}")
        cloudlog.debug(f"steer_angle={actuators.steeringAngleDeg:.2f}")
